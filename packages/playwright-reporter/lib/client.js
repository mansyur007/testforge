// Minimal TestForge REST client for streaming reporters. No dependencies —
// Node 18+ global fetch. Resolves TC-<SLUG>-<n> annotations to case ids by
// pulling the project's cases once, then creates a run, streams results, and
// completes the run.
...
export class TestForgeClient {
  constructor({ url, token, project }) {
    ...
    this.caseMap = new Map(); // "TC-WEB-001" (upper) -> case id
    this.runId = null;
  }
  ...
  /** Page through the project's cases, building displayId -> id. */
  async loadCaseMap() {
    let cursor = null;
    do {
      ...
      const page = await this.#req(
        `/projects/${this.project}/cases?${qs}`
      );
  ...
  /** Extract & resolve a TC id from any test-name-ish string. */
  resolveCaseId(text) {

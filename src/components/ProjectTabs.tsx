    ...
    <div>
      ...
      <div className="mt-4 flex items-stretch gap-1 border-b border-slate-200">
        ...
        <GroupMenu
          ...
          activeKey={active}
        />
        {/* Same tab-underline treatment as the rest of the row. Opens the
            settings modal route, which server-renders the section straight
            into it. */}
        <Link
          href={settingsHref(slug)}
          data-testid="project-settings-trigger"
          className={`inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap border-b-2 px-4 py-2 text-sm font-medium ${FOCUS_RING} ${
            settingsActive
              ? "border-indigo-600 text-indigo-600"
              : "border-transparent text-slate-500 hover:text-slate-800"
          }`}

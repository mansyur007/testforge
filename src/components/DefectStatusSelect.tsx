...
export function DefectStatusSelect({
  ...
  return (
    <select
      ...
      className={`cursor-pointer rounded-lg border-0 px-2 py-1 text-xs font-medium focus:ring-2 focus:ring-accent-ring disabled:opacity-50 motion-safe:transition-colors motion-safe:duration-fast motion-safe:ease-tf-out ${DEFECT_STATUS_BADGES[status as DefectStatus] ?? DEFECT_STATUS_BADGES.OPEN} ${className}`}
    >

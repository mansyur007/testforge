        ...
        {items.map((v) => (
          <div
            ...
            className={`group flex items-center gap-1 rounded-lg px-2 py-1 ${
              v.id === activeViewId ? "bg-accent-soft" : "hover:bg-surface-muted"
            }`}
          >
            <button
              type="button"
              ...
              <form action={toggleDefaultSavedView}>
                ...
                <button
                  ...
                  className={`px-1 text-sm ${
                    v.isDefault ? "text-warning" : "text-content-subtle hover:text-warning"
                  }`}
                >
                  ★
                </button>
            ...
            {(v.mine || (v.shared && canManageShared)) && (
              <form action={deleteSavedView}>
                ...
                <button
                  type="submit"
                  title="Delete view"
                  className="hidden px-1 text-sm text-content-subtle hover:text-danger group-hover:block"
                >
                  ×
                </button>
              </form>
    ...
    <div ref={rootRef} className="relative">
      <button
        ...
        className={`rounded-lg border px-3 py-2 text-sm hover:bg-surface-muted ${
          active ? "border-accent-ring text-accent-soft-fg" : "border-hairline-strong"
        }`}
      >
      ...
      {open && (
        <div
          ...
          className="absolute left-0 z-40 mt-1 w-72 origin-top-left rounded-xl border border-hairline bg-surface p-2 shadow-xl motion-safe:animate-tf-pop-in"
        >
          <button
            type="button"
            ...
            className={`w-full rounded-lg px-2 py-1.5 text-left text-sm ${
              !active ? "bg-canvas font-medium" : "hover:bg-surface-muted"
            }`}
          >
            All cases
          </button>
            ...
            {saving ? (
              <form action={formAction} className="space-y-2 px-1">
                ...
                <input
                  ...
                  className="bg-surface text-content-strong w-full rounded-lg border border-hairline-strong px-2 py-1.5 text-sm focus:border-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-ring"
                />
                <div className="flex items-center gap-3 text-xs text-content">
                  {canShare && (
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="rounded-lg px-3 py-1.5 text-xs text-content-muted hover:bg-surface-muted"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <button
                ...
                className="w-full rounded-lg px-2 py-1.5 text-left text-sm text-accent-text hover:bg-accent-soft disabled:cursor-not-allowed disabled:text-content-subtle"
              >
